...
function gogh {
  ...
  if [ -z "$repo" ]; then
    ...
    repo=$(
      curl -s -X GET "https://api.github.com/search/repositories?q=$repo&fork=false&in=html_url&order=desc" | \
      python -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["items"][0]["html_url"])'
    )
  else
